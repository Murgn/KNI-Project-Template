...
public class Tilemap
{
    ...
    public void Draw(SpriteBatch spriteBatch, Vector2 position = default)
    {
        ...
        {
            int index = tiles[i];
            TextureRegion tile = tileset.GetTile(index);

            int x = i % Columns;
            int y = i / Columns;

            Vector2 pos = new Vector2(x * TileWidth, y * TileHeight) + position;
            tile.Draw(spriteBatch, pos, Color, 0.0f, Vector2.Zero, Scale, SpriteEffects.None, 1.0f);
        }
    }

    public void SetTile(int index, int ID) => tiles[index] = ID;

    public void SetTile(int column, int row, int ID)
    {
        int index = row * Columns + column;
        SetTile(index, ID);
    }
    
    public TextureRegion GetTile(int index) => tileset.GetTile(index);
        ...
        using (Stream stream = TitleContainer.OpenStream(filePath))
        {
            using (XmlReader reader = XmlReader.Create(stream))
            {
                ...
                for (int row = 0; row < rows.Length; row++)
                {
                    ...
                    {
                        int index = int.Parse(columns[column]);
                        tilemap.SetTile(column, row, index);
                    }
                }
